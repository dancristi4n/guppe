...
# random() -> Gera um número real pseudo-aleatorio entre 0 e 1.

# Ao realizar o import de todo o módulo, todas as funções atributos, classes e propriedades que estiverem dentro do módulo ficarão disponíveis (ficarão em Memória). Caso você saiba quais funções utilizar deste módulo, então esta não seria a forma ideal de utilização. Nós veremos uma forma melhor na Forma 2.

print(random.random())

# Veja que para utilizar função random() do pacote random, nós colocamos o nome do pacote e o nome da função separados por ponto.
